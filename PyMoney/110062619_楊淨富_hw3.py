...
class Record:
  ...
  Represent a record.
  """
  def __init__(self, category, item, amount, add_date, add_time):
    """
...
class Records:
    ...
    try:
      with open('records.txt', 'r+') as fh:
        ...
        for line in fh.readlines():
          try:
            L = line.split()
            cate, item, amount, add_date, add_time = L[0], L[1], L[2], L[3], L[4]
            new_record = Record(cate, item, amount, add_date, add_time)
  ...
  def add(self, record, catogories):
    ...
    while True:
      try:
        ...
        add_date = current_time.strftime("%Y-%m-%d")
        add_time = current_time.strftime("%H:%M:%S")
        try:
          amount = int(amount)
        except ValueError as err: 
          sys.stderr.write(str(err) + '\n' + 'Try again.\n')
        ...
        if categories.is_category_valid(category):
          new_record = Record(category, item, amount, add_date, add_time)
          self._records.append(new_record)
          break
        else:
  ...
  def view(self):
    ...
    print("%-20s %-20s %-10s %-10s %-10s" % ("Category", "Desciption", "Amount", "Date", "Time")) # left-aligned
    print("="*20 + " " + "="*20 +  " " + "="*10 + " " + "="*10 + " " + "="*10)
    total = self._initial_money
    line_no = 0
    for each_rec in self._records:
      cate, item, amount, add_date, add_time = each_rec._category, each_rec._item, each_rec._amount, each_rec._add_date, each_rec._add_time
      total += int(amount)
      line_no += 1
      print(f'{line_no:>2}.{cate:<17s} {item:<20s} {amount:<10} {add_date:<10s} {add_time:<10s}')
    print("="*20 + " " + "="*20 + " " + "="*10 + " " + "="*10 + " " + "="*10)
    print(f'Now you have {total} dollars.')

  def sort_by_amount(self):
    """
    Print all the records in increasing order of amount and report the balance.
    """
    print("Here's your expense and income records:")
    print("%-20s %-20s %-10s %-10s %-10s" % ("Category", "Desciption", "Amount", "Date", "Time"))
    print("="*20 + " " + "="*20 +  " " + "="*10 + " " + "="*10 + " " + "="*10)
    total = self._initial_money
    line_no = 0
    for each_rec in sorted(self._records, key = lambda x: int(x._amount)):
      cate, item, amount, add_date, add_time = each_rec._category, each_rec._item, each_rec._amount, each_rec._add_date, each_rec._add_time
      total += int(amount)
      line_no += 1
      print(f'{line_no:>2}.{cate:<17s} {item:<20s} {amount:<10} {add_date:<10s} {add_time:<10s}')
  ...
  def save(self):
    ...
    with open('records.txt', mode = 'w') as fh:
      ...
      for rec in self._records:
        s = rec._category + " " + rec._item + " " + str(rec._amount) + " " + rec._add_date + " " + rec._add_time
        fh.write(s + '\n')
...
if __name__ == '__main__':
  ...
  while True:
    command = input('\nWhat do you want to do (add / view / delete / revise / view categories / find / sort amount / exit)? ')
    if command == 'add':
    ...
    elif command == 'sort amount':
      records.sort_by_amount()
    elif command == 'exit':
      records.save()
      break
    else:
